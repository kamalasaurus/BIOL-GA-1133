...
```{r}
summary_stats %>% arrange(Diet, Time) %>% head(20)
```
...
ggplot(summary_stats, aes(x = Time, y = mean_weight, color = Diet)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = mean_weight - se, ymax = mean_weight + se), width = 0.3) +
...
```{r}
m1 <- lm(weight ~ Time, data = ChickWeight)         #Basic
m2 <- lm(weight ~ Time + Diet, data = ChickWeight)  #Diet+Time
m3 <- lm(weight ~ Time * Diet, data = ChickWeight)  #Diet*Time

rss1 <- calculate_rss(m1)
rss2 <- calculate_rss(m2)
rss3 <- calculate_rss(m3)
...
```{r}
full_model <- lm(weight ~ 0 + Time*Diet, data = ChickWeight)
...
f_test_backward_selection <- function(full_model) {
  ...
  while (improved) {
    improved <- FALSE
    terms <- attr(current_model$terms, "term.labels")
    
    #Loop through each variable
    for (term in terms) {
      reduced_model <- update(current_model, paste(". ~ . -", term))
      test <- anova(reduced_model, current_model)
      
      #If delete this variable, P value is greater than 0.05 → can delete it
      if (test$`Pr(>F)`[2] > 0.05) {
...
quad_model   <- lm(weight ~ Time + I(Time^2), data = ChickWeight)

rss_linear <- sum(resid(linear_model)^2)
rss_quad   <- sum(resid(quad_model)^2)

c(rss_linear, rss_quad)
```
